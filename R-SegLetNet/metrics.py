...
class FocalLoss2d(nn.Module):
    def __init__(self, alpha=0.5, gamma=2, weight=None, ignore_index=255, size_average=True):
        ...
        self.ce_fn = crossentry()# nn.CrossEntropyLoss(weight=self.weight, ignore_index=self.ignore_index)
...
class SoftDiceMeanLoss(nn.Module):
    def __init__(self):
        super(SoftDiceMeanLoss, self).__init__()

    def forward(self, logits, targets):
        logits = torch.sigmoid(logits)  # 应用 softmax
        class_num = logits.size(1)
        dice_sum = 0
