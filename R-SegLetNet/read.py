...
Path_img_train = './data/leftImg8bit/train/'

# +++++++++++++++++++++++++++++++++++
Path_img_test = './data/leftImg8bit/test/berlin/'

# 原来这的lable是训练集的lable
Path_lab = './data/gtFine/train/'
...
#     for file in files:
#         # 检查文件名是否以指定后缀结尾
#         if file.endswith('gtFine_color.png'):
#             found_path = os.path.join(root, file)
#             img_test.append(found_path)
...
class Data(Dataset.Dataset):
    ...
    def __getitem__(self, index):
        ...
        second_last_slash = self.img[index].rfind('/', 0, self.img[index].rfind('/'))
        ins = self.img[index].rfind('/')
        # print(self.img[index])
        # print(self.img[index][ins:-15])
        desired_string = self.img[index][second_last_slash + 1:ins]
        labelname = Path_lab + desired_string + self.img[index][ins:-15] + 'gtFine_labelIds.png'
        # print(labelname)
        label = io.imread(labelname)
        label_one = convert_to_one_hot(label)
