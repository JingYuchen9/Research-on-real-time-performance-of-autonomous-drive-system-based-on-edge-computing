...
class ModelWithLoss(nn.Module):
    ...
    def forward(self, imgs, annotations, seg_annot, obj_list=None):
        ...
        if self.debug:
            ...
            # print(regression.device, classification.device, anchors.device, annotations.device)
            # print(f"Criterion device: {next(self.criterion.parameters()).device}")
            # print(f"Seg_criterion1 (Tversky) device: {next(self.seg_criterion1.parameters()).device}")
            # print(f"Seg_criterion2 (Focal) device: {next(self.seg_criterion2.parameters()).device}")
            cls_loss, reg_loss = self.criterion(classification, regression, anchors, annotations)
            tversky_loss = self.seg_criterion1(segmentation, seg_annot)
            focal_loss = self.seg_criterion2(segmentation, seg_annot)
...
class BiFPN(nn.Module):
    def __init__(self, num_channels, conv_channels, first_time=False, epsilon=1e-4, onnx_export=False, attention=True,
        ...
        self.p4_downsample = Down_wt(160,160)
        self.p5_downsample = Down_wt(160,160)
        self.p6_downsample = Down_wt(160,160)
        self.p7_downsample = Down_wt(160,160)
...
class BiFPNDecoder(nn.Module):
    def __init__(
        ...
        self.seg_p2 = SegmentationBlock(seg_p2_in_channels, 64, n_upsamples=0)
