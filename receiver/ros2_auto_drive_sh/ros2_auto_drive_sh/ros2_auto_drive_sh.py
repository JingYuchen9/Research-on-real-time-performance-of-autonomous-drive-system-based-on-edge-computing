...
###################### 第三次尝试 kmeans #################
    ...
    def get_line_pos(self, img, Width ,Height, lines, cvmodel, left=False, right=False):
        global Offset, Gap
        if cvmodel:
            x_sum = 0.0
            y_sum = 0.0
    ...
    def process_image(self, frame, Height, Width, input_img):
        ...
        if all_lines is None:
            ...
            roicv = edge_img[Offset : Offset+Gap, 0:Width]
            cv_all_lines = cv2.HoughLinesP(roicv,1,math.pi/180,30,30,10)
            print("cv model")
            # cv2.imshow('roicv', roicv)
            # cv2.waitKey(1)
            cvmodel = True
            if cv_all_lines is None:
                return 0, 640
            all_lines = cv_all_lines
        left_lines, right_lines = self.divide_left_right(all_lines, Width)
        # get center of lines
        frame, lpos, left_slope = self.get_line_pos(frame, Width, Height, left_lines, cvmodel, left=True)
        frame, rpos, right_slope = self.get_line_pos(frame, Width, Height, right_lines, cvmodel, right=True)
