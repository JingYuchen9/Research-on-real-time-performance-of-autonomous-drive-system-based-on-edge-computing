...
class auto_driveSubscriber(Node):
    def __init__(self, name):
        ...
        # print(x.shape)
        weight = torch.load(weight, map_location='cuda' if self.use_cuda else 'cpu', weights_only=True)
        ...
        self.model.eval()
        ###############################################
        self.cap = cv2.VideoCapture('/home/jing/ros2_ws/src/ros2_auto_drive_sh/ros2_auto_drive_sh/1.mp4')
        ...
        # 自动驾驶节点
        self.camera_subscribe_  = self.create_subscription(ImageData, "ros2_auto_drive", self.auto_drive_callback, qos_profile)
        # self.camera_subscribe_  = self.create_subscription(ImageData, "ros2_camera_strengthen", self.auto_drive_callback, qos_profile)
        
        # 分割节点可视化
        self.seg_result  = self.create_subscription(ImageData, "seg_result", self.seg_result, qos_profile)
        
        # 电机节点
        self.motor_publish_ = self.create_publisher(Motor, "XycarMotor", qos_profile)

        # 返回RTT
        self.rtt_back = self.create_publisher(UInt32, "rtt", qos_profile)
    ...
    def avoidance(self, center_x, center_y, ):
...
########################### 第一次尝试, 取平均的平均 ################
    ...
    #     # Step 1: Calculate the average slope (m_avg) of all lines
    #     slopes = []
    #     for line in lines:
    #         x1, y1, x2, y2 = line[0]
    ...
    #     m_avg = sum(slopes) / size

    #     # Step 2: Filter lines with slope less than the average slope
    #     filtered_lines = []
    #     for i, slope in enumerate(slopes):
    #         if slope < m_avg:
    #             filtered_lines.append(lines[i])

    #     # Step 3: Calculate the average parameters (m, b) of the filtered lines
    ...
    #     x_avg = x_sum / (filtered_size * 2)
    #     y_avg = y_sum / (filtered_size * 2)
    #     m = m_sum / filtered_size
    #     b = y_avg - m * x_avg
...
###################### 第三次尝试 kmeans #################
    ...
    def process_image(self, frame, Height, Width):
        global Offset, Gap
        global time_rate
        ...
        cv2.imshow('roi', roi)
        cv2.waitKey(1)
        all_lines = cv2.HoughLinesP(roi,1,math.pi/180,20,10,5)
        ...
        # get center of lines
        frame, lpos = self.get_line_pos(frame, Width, Height, left_lines, left=True)
        frame, rpos = self.get_line_pos(frame, Width, Height, right_lines, right=True)
    ...
    def start(self):
        ...
        with torch.no_grad():
            ...
            lpos, rpos = self.process_image(lane_mask.astype(np.uint8), h, w)
            # if len(self.l_history) >= 5:
            #     self.l_history.pop(0)
            # if len(self.r_history) >= 5:
            #     self.r_history.pop(0)
            
            # self.l_history.append(lpos)
            # self.r_history.append(rpos)

            # l_smoothed = sum(self.l_history) / len(self.l_history)
            # r_smoothed = sum(self.r_history) / len(self.r_history)
...
################################## 物体检测 ######################################
            ...
            # if slope < 0 and target_angle <= 60:
            #     # angle = angle - 30
            #     print("触发左避让,物体在右边")
