...
class cameraPublisher(Node):
    def __init__(self, name):
        super().__init__(name)
        self.get_logger().info("ros2_camera节点被激活")
        ...
        self.command_publisher_ = self.create_publisher(ImageData, "ros2_camera", qos_profile)
        # 图像传输
        self.bridge = CvBridge()
        self.publish_image = np.empty(shape=[0])
    ...
    def player(self, event):
        ...
        while True:
            ...
            if rqt_sequence_number > self.point_frame:
                ...
                # self.command_publisher_.publish(publish_image)
                # frame_data_np = cv2.cvtColor(frame_data, cv2.COLOR_RGB2BGR)
...
def main(args=None):
    ...
    node = cameraPublisher("ros2_camera")  # 新建一个节点
