...
# 绘制平滑后的折线图
plt.plot(smoothed_data1, color='b', linestyle='-', label='blocksize=10')
# plt.plot(smoothed_data2, color='r', linestyle='-', label='blocksize=16')
plt.plot(smoothed_data3, color='r', linestyle='-', label='blocksize=20')
plt.plot(smoothed_data4, color='lime', linestyle='-', label='blocksize=40')
plt.plot(smoothed_data5, color='m', linestyle='-', label='blocksize=80', alpha=0.7)

# 设置x轴的范围
plt.xlim(0, min_length - 1)

# 添加标题、标签和图例
plt.title('Relationship between macroblock size and frame rate')
